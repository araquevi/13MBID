...
st.write("#### Persona/s: Víctor Araque Rodríguez")
st.write("----")
...
option = st.selectbox(
    'Qué tipo de crédito desea filtrar?',
     df['objetivo_credito_N'].unique())

df_filtrado = df[df['objetivo_credito_N'] == option]
...
if st.checkbox('Mostrar créditos finalizados?', value=True):
    ...
    falta_pago_counts = df_filtrado['falta_pago_N'].value_counts()
